    ...
    <x-content.container-fluid>
        ...
        <x-content.table-container>
            ...
            <x-content.table-body>
                ...
                <x-content.thead :items="['No', 'Kode Barang', 'Nama Barang', 'Jenis Barang', 'Satuan', 'Aksi']" />

                <x-content.tbody>
                    @foreach ($items as $item)
                        <x-content.table-row :index="$loop->index + 1" :item="$item" :columns="[$item->item_code, $item->name, $item->itemType->name, $item->unitType->name]" :actions="[
